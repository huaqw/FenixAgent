...
const app = new Elysia({ name: "web-teams", prefix: "/web" })
  .use(authGuardPlugin);

app.post("/teams", async ({ store, body, error, request }: any) => {
  const b = (body as any) ?? {};
  ...
  switch (b.action) {
    ...
    case "create": {
      if (!b.name || !b.slug) return error(400, { success: false, error: { code: "VALIDATION_ERROR", message: "name and slug required" } });
      try {
    ...
    case "update": {
      if (!authCtx) return error(500, { success: false, error: { code: "NO_TEAM_CONTEXT" } });
      if (!["owner", "admin"].includes(authCtx.role)) return error(403, { success: false, error: { code: "FORBIDDEN", message: "owner or admin only" } });
      const t = await teamService.updateTeam(b.teamId ?? authCtx.teamId, b.data);
      return { success: true, data: t };
    }
    case "delete": {
      if (!authCtx) return error(500, { success: false, error: { code: "NO_TEAM_CONTEXT" } });
      if (authCtx.role !== "owner") return error(403, { success: false, error: { code: "FORBIDDEN", message: "owner only" } });
      const ok = await teamService.deleteTeam(authCtx.teamId);
      return { success: true, data: { deleted: ok } };
    }
    case "switch": {
      if (!b.teamId) return error(400, { success: false, error: { code: "VALIDATION_ERROR", message: "teamId required" } });
      const { getAuthContextByTeamId } = await import("../../services/team");
      const ctx = await getAuthContextByTeamId(user.id, b.teamId);
      if (!ctx) return error(403, { success: false, error: { code: "FORBIDDEN", message: "Not a member of this team" } });
    ...
    case "add-member": {
      if (!authCtx) return error(500, { success: false, error: { code: "NO_TEAM_CONTEXT" } });
      if (!["owner", "admin"].includes(authCtx.role)) return error(403, { success: false, error: { code: "FORBIDDEN", message: "owner or admin only" } });
      if (!b.userId) return error(400, { success: false, error: { code: "VALIDATION_ERROR", message: "userId required" } });
    ...
    case "update-role": {
      if (!authCtx) return error(500, { success: false, error: { code: "NO_TEAM_CONTEXT" } });
      if (authCtx.role !== "owner") return error(403, { success: false, error: { code: "FORBIDDEN", message: "owner only" } });
      if (!b.userId || !b.role) return error(400, { success: false, error: { code: "VALIDATION_ERROR", message: "userId and role required" } });
