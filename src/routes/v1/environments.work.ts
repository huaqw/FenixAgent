...
const app = new Elysia({ name: "v1-environments-work", prefix: "/v1/environments" })
  .use(authGuardPlugin);
...
app.post("/:id/work/:workId/ack", async ({ params }) => {
...
app.post("/:id/work/:workId/stop", async ({ params }) => {
  const workId = params.workId;
  stopWork(workId);
  return { status: "ok" };
}, { apiKeyAuth: true });
...
app.post("/:id/work/:workId/heartbeat", async ({ params }) => {
  const workId = params.workId;
